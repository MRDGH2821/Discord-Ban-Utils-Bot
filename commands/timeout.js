  ...
  async execute(interaction) {
    ...
    const duration = interaction.options.getInteger('duration'),
      ...
      reason =
        ...
        `Timed-out by ${interaction.user.tag} for ${timeoutDurationText(
          duration
    ...
    try {
      canTimeout = await interaction.member.permissions.has([
      ...
      if (isInGuild && canTimeout) {
        ...
        if (duration > NUMBER.zero) {
          const timeout_success = new MessageEmbed()
            ...
            .setDescription(
              `User \`${target.user.tag}\` ${target} is put into timeout.`
            )
          ...
          const timeout_removed = new MessageEmbed()
            ...
            .setDescription(
              `User \`${target.user.tag}\` ${target} is out of timeout.`
            )
            .setTimestamp();
      ...
      const timeout_fail = new MessageEmbed()
        ...
        .addFields([
          ...
          {
            name: '**Inputs given**',
            value: `User: ${target} \nTimeout duration: ${timeoutDurationText(
              duration
            )}\nReason: ${reason}`
          },
          {
