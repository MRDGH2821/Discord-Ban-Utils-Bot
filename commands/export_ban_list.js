			...
			if (interaction.guild) {
				...
				function(success, data) {
					if (success) {
						...
						return interaction.editReply({ content:data, components:[InviteRow] });
					}
					else {
						// Incase of any errors
						return interaction.editReply({ content:'There was some unexpected error.', components:[SupportRow] });
					}
				});
			}
			else {
				...
				if (Object.keys(servers).length > 0) {
					...
					collector.on('collect', async (i) => {
						if (i.user.id === interaction.user.id) {
							...
							initial_Screen
								...
								.setFooter('Please wait for a while even if it says interaction failed');
							await interaction.editReply({ embeds:[initial_Screen], components:[], fetchReply:true });
						}
						else {
							interaction.reply({ content:'These commands are not for you!', ephemeral: true });
						}
					});
					...
					collector.on('end', async (collected) => {
						if (collected.size === 1) {
							initial_Screen
								.addFields({ name:'\u200b', value:`Found ${bans.length} bans. Exporting...` })
								.setFooter('');
							await interaction.editReply({ embeds:[initial_Screen], components:[InviteRow], fetchReply: true });
							console.log(`Found ${bans.length} bans. Exporting...`);
							console.log(bans);
							...
							await interaction.editReply({ embeds:[initial_Screen], components:[InviteRow] });
						}
					},
					);
				}
				...
				// When mutual servers are less than 1
					initial_Screen
						.setDescription('No Mutual servers Found. Invite the bot in your server to use this command!');
					await interaction.editReply({ embeds: [initial_Screen], components:[InviteRow], fetchReply: true });
				}
			}
		}
		catch (e) {
			interaction.reply({ content:`Unexpected error occured, please report it to the developer! \nError dump:\n\n\`${e}\``, components:[SupportRow] });
		}
	},
};
